  ...
  <div class="page">
    <AppHeader :title="doc?.title || 'Document'" back>
      <template #default>
        ...
        <button v-if="hasToc" class="hbtn" aria-label="Contents" @click="tocOpen = true">
    ...
    <div v-if="doc" class="viewer">
      <ScriptureView
        v-if="doc.type === 'scripture'"
        :doc-id="doc.id"
        :book="goToBook"
        :chapter="goToChapter"
      />
      <PdfViewer v-else-if="doc.type === 'pdf'" :src="doc.file" :doc-id="doc.id" :go-to="goToPage" />
...
import ScriptureView from '@/components/ScriptureView.vue'
...
const hasToc = computed(() => doc.value && doc.value.type !== 'hymns')
const goToPage = computed(() => parseInt(route.query.page) || 0)
const goToBook = computed(() => route.query.book || '')
const goToChapter = computed(() => parseInt(route.query.chapter) || 0)
const tocOpen = ref(false)

function onTocSelect(sel) {
  tocOpen.value = false
  if (sel && typeof sel === 'object' && 'book' in sel) {
    router.replace({ query: { book: sel.book, chapter: String(sel.chapter) } })
  } else {
    router.replace({ query: { page: sel } })
  }
}
</script>
